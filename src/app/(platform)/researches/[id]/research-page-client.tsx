    ...
    <div className="flex-1 overflow-auto" style={{ background: "#fff" }}>
      <div className="p-6 max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-2 text-xs mb-5" style={{ color: "#8b7355" }}>
          <Link href="/dashboard" className="hover:underline" style={{ color: "#b07d20" }}>Dashboard</Link>
          <i className="ti ti-chevron-right text-xs" />
          <Link href="/researches" className="hover:underline" style={{ color: "#b07d20" }}>Pesquisas</Link>
          <i className="ti ti-chevron-right text-xs" />
          <span style={{ color: "#5c4a2a" }}>{research.title}</span>
        </div>
        ...
        <div className="flex items-start justify-between mb-6">
          <div className="flex-1 min-w-0 mr-4">
            ...
            {research.description && (
              <p className="text-sm mt-1 font-medium" style={{ color: "#5c4a2a" }}>{research.description}</p>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="md:col-span-2 flex flex-col gap-4">
            ...
            <div className="rounded-xl p-5" style={{ border: BRD, background: "#fff" }}>
              <div className="flex items-center gap-2 mb-4">
                <div className="w-7 h-7 rounded-md flex items-center justify-center" style={{ background: "#fff8ec" }}>
                  <i className="ti ti-world text-sm" style={{ color: "#b07d20" }} />
                </div>
                <h2 className="text-sm font-bold" style={{ color: "#0a1628" }}>Publicação</h2>
              </div>

              {/* Toggle coleta */}
              <div className="flex items-center justify-between p-3 rounded-lg mb-4"
                style={{ background: "#faf6ef", border: BRD }}>
                <div>
                ...
                <div className="flex gap-2">
                  {status !== "active" && status !== "published" ? (
                    <button onClick={() => toggleStatus("active")} disabled={saving}
                      ...
                      style={{ background: "#0d9e75", color: "#fff" }}>
                      <i className="ti ti-player-play" /> Ativar coleta
                    </button>
                  ) : (
                    <button onClick={() => toggleStatus("paused")} disabled={saving}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-bold disabled:opacity-50"
                      style={{ background: "#faeeda", color: "#854f0b", border: "1px solid #e8c870" }}>
                ...
                <div className="flex gap-2">
                  {[
                    ...
                    <button key={mode.key} onClick={() => setAccessMode(mode.key as "public" | "restricted")}
                      ...
                      style={{
                        border: accessMode === mode.key ? "1.5px solid #b07d20" : BRD,
                        background: accessMode === mode.key ? "#fff8ec" : "#faf6ef",
                        color: accessMode === mode.key ? "#7a3d00" : "#5c4a2a",
                      }}>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={copyLink}
                      ...
                      style={{ background: copied ? "#0d9e75" : "#b07d20", color: "#fff" }}>
                  ...
                  {showQR && (
                    <div className="mt-3 p-4 rounded-lg flex flex-col items-center gap-2"
                      ...
                      <div className="w-32 h-32 rounded-lg flex items-center justify-center"
                        ...
                        <i className="ti ti-qrcode text-5xl" style={{ color: "#b07d20" }} />
                      </div>
                      <p className="text-xs font-semibold" style={{ color: "#5c4a2a" }}>QR Code do formulário</p>
              ...
              {accessMode === "restricted" && (
                <div>
                  ...
                  {invites.length === 0 ? (
                    <p className="text-xs py-3 text-center" style={{ color: "#8b7355" }}>Nenhum pesquisador convidado ainda</p>
                  ) : (
                      ...
                      {invites.map(email => (
                        <div key={email} className="flex items-center justify-between px-3 py-2 rounded-md text-xs"
                          ...
                          <div className="flex items-center gap-2">
                            <i className="ti ti-user-circle" style={{ color: "#b07d20" }} />
                            <span style={{ color: "#5c4a2a" }}>{email}</span>
                          </div>
            ...
            <div className="rounded-xl p-5" style={{ border: BRD, background: "#fff" }}>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-7 h-7 rounded-md flex items-center justify-center" style={{ background: "#e1f5ee" }}>
                  <i className="ti ti-test-pipe text-sm" style={{ color: "#0d9e75" }} />
                </div>
                <div>
                  <h2 className="text-sm font-bold" style={{ color: "#0a1628" }}>Testar formulário</h2>
                  <p className="text-xs" style={{ color: "#8b7355" }}>As respostas de teste não são salvas</p>
                </div>
              </div>
              <div className="flex gap-2">
          ...
          <div className="flex flex-col gap-4">
            ...
            {/* Métricas */}
            <div className="rounded-xl p-4" style={{ border: BRD, background: "#faf6ef" }}>
              ...
              {[
                ...
                <div key={m.label} className="flex items-center justify-between py-2"
                  ...
                  <div className="flex items-center gap-2">
                    <i className={`ti ${m.icon} text-sm`} style={{ color: "#b07d20" }} />
                    <span className="text-xs font-medium" style={{ color: "#5c4a2a" }}>{m.label}</span>
                  </div>
                  <span className="text-sm font-bold" style={{ color: "#0a1628" }}>{m.val}</span>
                </div>
              ))}
            </div>
              ...
              <div className="flex flex-col gap-2">
                {[
                  ...
                  <div key={cfg.label} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <i className={`ti ${cfg.icon} text-sm`} style={{ color: "#b07d20" }} />
                      <span className="text-xs font-medium" style={{ color: "#5c4a2a" }}>{cfg.label}</span>
                    </div>
                    <div className="w-7 h-3.5 rounded-full relative" style={{ background: cfg.active ? "#b07d20" : "#e8d9c0" }}>
