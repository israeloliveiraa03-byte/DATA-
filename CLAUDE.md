...
**Custo de operar (excluindo IA, ainda não decidida)**: infra fixa (Vercel + Neon + storage + e-mail) ≈ R$1.300/mês em estágio inicial-crescimento; custo variável por conta pagante ≈ R$4/mês (gateway de pagamento + banda/armazenamento incremental). Com ticket médio de R$35, margem de contribuição ≈ 89%; breakeven do fixo em ≈42 contas pagantes.

**Gateway escolhido: Asaas** (decisão de Israel, 2026-07-09) — Pix/boleto/cartão/NF-e nativos, melhor pro público brasileiro que Stripe puro (que não resolve Pix/boleto/NF-e nativamente, e universidade/governo vão exigir isso). Cadastro inicial com **CPF** (sem CNPJ ainda) — funciona pra validar o fluxo técnico, mas NF-e de serviço vai exigir migrar pra CNPJ antes de vender pro público institucional de verdade. `stripe` (pacote que estava instalado, nunca usado) foi removido do `package.json` em 2026-07-09.

**Status da implementação (fase 1, 2026-07-09)**: `users.plan` corrigido pros 5 tiers reais (`free|pesquisador|laboratorio|governo|territorio`, renomeado via `ALTER TYPE ... RENAME VALUE` a partir do enum antigo `free/pro/institution` — nenhuma linha de produção usava `pro`/`institution` além de `free`, confirmado antes de rodar). Tabelas `subscriptions`/`payments` (`src/lib/db/schema/billing.ts`) e cliente Asaas (`src/lib/asaas/client.ts`) prontos mas **não conectados a nenhuma tela ainda** — faltava a chave de API, que Israel ainda vai gerar no sandbox. Endpoint `/api/webhooks/asaas` escrito com base na documentação oficial (header `asaas-access-token`, eventos `PAYMENT_RECEIVED`/`PAYMENT_CONFIRMED`), mas **não testado de ponta a ponta** — sem token real ainda.

**Pendente antes de cobrar de verdade (fase 2)**: tela de upgrade de plano (chama `createAsaasSubscription`, mostra QR Pix/link), portal de autoatendimento (ver fatura, trocar meio de pagamento, cancelar), granularidade do anúncio por dashboard avulso (hoje `showAds` só olha `plan === "free"` no total, não por dashboard), teste real do webhook contra evento do sandbox.
